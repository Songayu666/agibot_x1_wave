...
bool ControlModule::Initialize(aimrt::CoreRef core) {
  ...
  try {
    if (!file_path.empty()) {
      ...
      // Import selected action controllers from one authoritative YAML file.
      // Simulation may then scale only the PD gains while sharing exactly the
      // same joint lists, safety limits, and trajectories as the real robot.
      if (cfg_node["controller_import_file"]) {
        const auto import_path = cfg_node["controller_import_file"].as<std::string>();
        const YAML::Node imported_cfg = YAML::LoadFile(import_path);
        const auto import_names =
            cfg_node["controller_imports"].as<std::vector<std::string>>();
        const double stiffness_scale =
            cfg_node["controller_import_stiffness_scale"]
                ? cfg_node["controller_import_stiffness_scale"].as<double>()
                : 1.0;
        const double damping_scale =
            cfg_node["controller_import_damping_scale"]
                ? cfg_node["controller_import_damping_scale"].as<double>()
                : 1.0;

        for (const auto& name : import_names) {
          if (!imported_cfg["controllers"][name]) {
            throw std::runtime_error("Imported controller not found: " + name);
          }
          YAML::Node controller = YAML::Clone(imported_cfg["controllers"][name]);
          auto stiffness = controller["stiffness"].as<std::vector<double>>();
          auto damping = controller["damping"].as<std::vector<double>>();
          for (auto& value : stiffness) value *= stiffness_scale;
          for (auto& value : damping) value *= damping_scale;
          controller["stiffness"] = stiffness;
          controller["damping"] = damping;
          cfg_node["controllers"][name] = controller;
        }
      }
